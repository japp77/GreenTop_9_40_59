...
If ((WHERE_TROVA_PEDANA = 2) Or (WHERE_TROVA_PEDANA = 3)) Then
    sSQL = sSQL & " AND ((RV_POAssegnazioneMerce.IDOggettoOrdine=" & FrmMain.txtIDOrdinePadre.Value & ") OR (RV_POAssegnazioneMerce.IDOggettoOrdine IS NULL))"
End If
...
Private Sub GridPedana_DblClick()
    ...
    End If
    If WHERE_TROVA_PEDANA = 3 Then
        frmSpezzaturaVeloce.txtIDPedana.Value = Me.GridPedana("IDRV_POPedana")
        frmSpezzaturaVeloce.txtCodicePedana.Text = Me.GridPedana.AllColumns("Codice").Value
    End If
