...
sSQL = sSQL & "RV_POPedana.IDRV_POTipoPedana , RV_POPedana.Descrizione, RV_POTipoPedana.TipoPedana, RV_POAssegnazioneMerce.NumeroListaPrelievo "
...
If frmMain.txtIDOrdineCliente.Value > 0 Then
    sSQL = sSQL & " AND ((RV_POAssegnazioneMerce.IDOggettoOrdine=" & frmMain.txtIDOrdineCliente.Value & ") OR (RV_POAssegnazioneMerce.IDOggettoOrdine IS NULL))"
End If
...
If WHERE_TROVA_PEDANA = 1 Then
    ...
    LINK_ORDINE_RIF = fnNotNullN(Me.GridPedana.AllColumns("IDOggettoOrdine").Value)
    Unload Me
End If
