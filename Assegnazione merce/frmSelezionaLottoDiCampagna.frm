...
On Error GoTo ERR_SettaggioGriglia
    ...
    Else
        If (GET_CONTROLLO_SOCI_PER_CLIENTE(frmMain.cdCliente.KeyFieldID) = 0) Then
            sSQL = "SELECT * FROM RV_PO01_IELottoDiCampagnaPerSelezione "
            sSQL = sSQL & " WHERE IDAzienda=" & TheApp.IDFirm
            If Me.chkLottoChiuso.Value = vbUnchecked Then
                sSQL = sSQL & " AND Chiuso=" & fnNormBoolean(Me.chkLottoChiuso.Value)
        ...
        Else
            ...
            sSQL = sSQL & " WHERE IDAzienda=" & TheApp.IDFirm
            sSQL = sSQL & " AND IDAnagraficaCliente=" & frmMain.cdCliente.KeyFieldID
            If Me.chkLottoChiuso.Value = vbUnchecked Then
                sSQL = sSQL & " AND Chiuso=" & fnNormBoolean(Me.chkLottoChiuso.Value)
            End If
            If Len(Trim(Me.txtRicerca.Text)) > 0 Then
...
Private Sub Form_Load()
    ...
    Me.chkTuttiLottiProd.Enabled = GET_CONTROLLO_SOCI_PER_CLIENTE(frmMain.cdCliente.KeyFieldID) > 0
    
End Sub
Private Sub Griglia_DblClick()
    LINK_LOTTO_PROD_LAV = fnNotNullN(Me.Griglia.AllColumns("IDRV_PO01_LottoCampagna").Value)
    frmMain.txtRaggrOrd.Text = frmMain.GET_DESCRIZIONE_LOTTO_PROD_LAV(LINK_LOTTO_PROD_LAV)
    Unload Me
End Sub
