...
Public Function ElaborazioneLiquidazioneSulVenduto(prg As ProgressBar)
    TIPO_QUADRATURA = ParametroTipoQuadratura
    ParametroSocio
    TotaleRighe = GET_TOTALERIGHE
    
    If TotaleRighe > 0 Then
        fnCalcolaPrezzoUnitario prg
        VenditeArticolo prg
    Else
        MsgBox "Non risultano dati per i filtri impostati", vbInformation, "Elaborazione dati"
    End If
End Function
Private Sub VenditeArticolo(prg As ProgressBar)
Dim sSQL As String
...
If GET_ANAGRAFICA_SOCIO(fnNotNullN(rsFatt!RV_POIDSocio)) = True Then
    If fnNotNullN(rsFatt!Link_Art_Articolo) > 0 Then
    
        If fnNotNullN(rsFatt!RV_POIDConferimentoRighe) > 0 Then
            If EsistenzaArticoloConferitoElaborato(fnNotNullN(rsFatt!RV_POIDConferimentoRighe)) = False Then
                ConferimentoRighe fnNotNullN(rsFatt!RV_POIDConferimentoRighe), fnNotNullN(rsFatt!RV_POIDSocio)
            End If
        End If
...
Set rsFatt = Nothing
    ...
    ConferimentoRigheSenzaRiferimenti
...
End If
End Function
Private Sub ConferimentoRighe(IDRigaConferimento As Long, IDSocio As Long)
Dim sSQL As String
...
Dim Link_CategoriaMerceologica As Long
    ...
    rsRighe.Open sSQL, CnDMT.InternalConnection
...
sSQL = sSQL & "WHERE IDRV_POCaricoMerceRighe=" & IDRigaConferimento
sSQL = sSQL & " AND IDRV_POPeriodoLiquidazione=" & LINK_PERIODO
...
While Not rsScarti.EOF
    If ARTICOLI_DI_QUAD = False Then
        TrattenuteArticolo IDSocio, rsScarti!IDArticolo, 0, fnNotNullN(rsScarti!IDTipoLavorazione), True, rsScarti!Qta_UM, 0
...
''''ELIMINAZIONE RIGHE TEMPORANEE DI ELABORAZIONE'''''
sSQL = "DELETE FROM RV_POTMPDocEla"
CnDMT.Execute sSQL
'''''''''''''''''''''''''''''''''''''''''''''''''''''''
...
''''''''''''''''''NOTA DI CREDITO''''''''''''''''''''
sSQL = "SELECT ValoriOggettoDettaglio0016.IDOggetto "
...
sSQL = sSQL & " WHERE ValoriOggettoPerTipo000B.Doc_Data>=" & fnNormDate(DATA_INIZIO)
sSQL = sSQL & " AND ValoriOggettoPerTipo000B.Doc_Data<=" & fnNormDate(DATA_FINE)
...
While Not rs.EOF
    ...
    If fnControlloEsistenzaDocTMP(fnNotNullN(rs!IDOggetto)) = False Then
         Link_Oggetto = fnNotNullN(rs!IDOggetto)
        'Prende in considerazione le righe dell'oggetto
        sSQL = "SELECT IDOggetto, IDValoriOggettoDettaglio, Art_prezzo_unitario_neutro "
        sSQL = sSQL & "FROM ValoriOggettoDettaglio0016 "
        sSQL = sSQL & "WHERE IDOggetto=" & fnNotNullN(rs!IDOggetto)
        ...
        While Not rsOgg.EOF
            ...
            sSQL = "SELECT * FROM RV_POCommissioniPerDoc WHERE IDOggetto=" & fnNotNullN(rsOgg!IDOggetto)
            Set rsComm = CnDMT.OpenResultset(sSQL)
                Prezzo = fnNotNullN(rsOgg!Art_prezzo_unitario_neutro)
                While Not rsComm.EOF
                    Prezzo = Prezzo - ((Prezzo / 100) * fnNotNullN(rsComm!Percentuale))
                rsComm.MoveNext
                Wend
...
End Function
Private Sub ConferimentoRigheSenzaRiferimenti()
Dim sSQL As String
Dim rsRighe As ADODB.Recordset
Dim Link_CategoriaMerceologica As Long
    ...
    sSQL = sSQL & "FROM RV_POCaricoMerceRighe INNER JOIN "
    sSQL = sSQL & "RV_POCaricoMerceTesta ON RV_POCaricoMerceRighe.IDRV_POCaricoMerceTesta = RV_POCaricoMerceTesta.IDRV_POCaricoMerceTesta "
    sSQL = sSQL & "WHERE RV_POCaricoMerceTesta.DataDocumento>=" & fnNormDate(DATA_INIZIO)
    sSQL = sSQL & " AND RV_POCaricoMerceTesta.DataDocumento<=" & fnNormDate(DATA_FINE)
    ...
    If EsistenzaArticoloConferitoElaborato(rsRighe!IDRV_POCaricoMerceRighe) = False Then
        Link_CategoriaMerceologica = GET_CATEGORIA_MERCEOLOGICA(fnNotNullN(rsRighe!IDArticolo))
...
End Function
Private Function GET_ANAGRAFICA_SOCIO(IDSocio As Long) As Boolean
Dim sSQL As String
Dim rs As DmtOleDbLib.adoResultset

sSQL = "SELECT  Anagrafica.IDAnagrafica "
...
sSQL = sSQL & "WHERE ((IDAzienda=" & TheApp.IDFirm & ") AND (Anagrafica.IDCategoriaAnagrafica=" & Link_TipoSocio & "))"
sSQL = sSQL & " AND (Anagrafica.IDAnagrafica=" & IDSocio & ")"

Set rs = CnDMT.OpenResultset(sSQL)
If rs.EOF Then
    GET_ANAGRAFICA_SOCIO = False
Else
    GET_ANAGRAFICA_SOCIO = True
End If
rs.CloseResultset
Set rs = Nothing
End Function
...
While Not rsFatt.EOF
    If fnNotNullN(rsFatt!Link_Art_Articolo) > 0 Then
        ...
        'TrattenuteArticolo IDSocio, fnNotNullN(rsFatt!Link_Art_Articolo), Link_CategoriaMerceologica, fnNotNullN(rsFatt!RV_POIDTipoLavorazione), False, fnNotNullN(rsFatt!Art_quantita_totale), fnNotNullN(PrezzoDaReg)
        
            sSQL = "INSERT INTO RV_POTMPLiquidazioneVendita ("
            sSQL = sSQL & "IDArticolo, CodiceArticolo, Articolo, IDRV_POCaricoMerceRighe, DataConferimento, IDSocio, "
            sSQL = sSQL & "IDRV_POPeriodoLiquidazione, Quantita, IDOggetto, IDValoreOggettoDettaglio, Oggetto, "
            sSQL = sSQL & "IDTipoOggetto, DataDocumentoVendita, NumeroDocumento, ImportoUnitario, ImportoNettoTotale, ImportoMedioPeriodo, ImportoTotaleSuPeriodo, "
            sSQL = sSQL & "IDIva_Vend, CodiceIva_Vend, AliquotaIva_Vend, Iva_Vend, IDIva_Medio, CodiceIva_Medio, AliquotaIva_Medio, Iva_Medio, "
            sSQL = sSQL & "ImpostaImportoUnitario, ImpostaImportoMedio, "
            ...
            sSQL = sSQL & "Colli, PesoLordo, PesoNetto, Tara, Pezzi, IDTipoLavorazione, IDCalibro, IDTipoCategoria, Quantita_Per_Totali) "
            'sSQL = sSQL & "IDImballo, CodiceImballo, Imballo, IDIva_Imballo, CodiceIva_Imballo, AliquotaIva_Imballo, Iva_Imballo, TaraUnitaria, QuantitaImballo, "
            'sSQL = sSQL & "Importo_unitario_imballo, Importo_uni_list_imballo, IDListino, Totale_netto_imballo, Totale_imposta_Imballo, "
            'sSQL = sSQL & "Totale_lordo_Imballo, Totale_netto_imballo_list, Totale_imposta_imballo_list, Totale_lordo_imballo_list) "
            sSQL = sSQL & "VALUES ("
            sSQL = sSQL & fnNotNullN(rsFatt!Link_Art_Articolo) & ", "
            ...
            sSQL = sSQL & fnNotNullN(rsFatt!RV_POIDTipoLavorazione) & ", "
            sSQL = sSQL & fnNotNullN(rsFatt!RV_POIDCalibro) & ", "
            sSQL = sSQL & fnNotNullN(rsFatt!RV_POIDTipoCategoria) & ", "
